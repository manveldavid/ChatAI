...
Usage: verify.py <username> <pin>
Returns JSON with success, owner, and owner_hash.
"""
...
def main():
    if len(sys.argv) < 3:
        print(json.dumps({"success": False, "error": "Usage: verify.py <username> <pin>"}))
        return
    ...
    if norm not in users:
        print(json.dumps({"success": False, "error": "not_found"}))
        return

    stored = users[norm]["pin_hash"]
    computed = hash_pin(pin)

    if stored == computed:
        print(json.dumps({"success": True, "owner": norm, "owner_hash": computed}))
    else:
        print(json.dumps({"success": False, "error": "wrong_pin"}))
